...
namespace ИЗ12
{
    class Database
    {
        ...
        public static void DownLoadDataForm5(int logIndex)
        {
            ...
            {
                dgv5.Rows.Add(database[logIndex].loginDates[i].Date.ToString(), database[logIndex].loginDates[i].TimeOfDay.Minutes.ToString()+ " : "+database[logIndex].loginDates[i].TimeOfDay.Seconds.ToString(), database[logIndex].logoutDates[i].TimeOfDay.Minutes.ToString()+ " : "+database[logIndex].logoutDates[i].TimeOfDay.Seconds.ToString(), (database[logIndex].logoutDates[i].TimeOfDay-database[logIndex].loginDates[i].TimeOfDay).Minutes.ToString()+ " : " + (database[logIndex].logoutDates[i].TimeOfDay - database[logIndex].loginDates[i].TimeOfDay).Seconds.ToString(), "");
            }
        }
    }
}
